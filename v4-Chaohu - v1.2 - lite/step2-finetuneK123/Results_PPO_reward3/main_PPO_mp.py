...
print(agent_params['training_step'])
...
if Train:
    ...
    for epoch in range(model.params['training_step']):
        ...
        np.save('./Results/Train'+str(finetuneID)+'.npy',history)
        if epoch % 10 == 0:
            t2.append(time.perf_counter())

    timeR=[ti-t1 for ti in t2]
    print('Running time: ',timeR)
    np.save('./time'+str(finetuneID)+'.npy',timeR)
    
    # plot
    plt.figure()
    plt.plot(history['Episode_reward'])
    plt.savefig('./Results/Train'+str(finetuneID)+'.tif')
