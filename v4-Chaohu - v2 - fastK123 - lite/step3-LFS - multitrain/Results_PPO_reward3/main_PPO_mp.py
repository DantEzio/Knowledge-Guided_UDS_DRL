...
for trainID in [42]:
    ...
    if Train:
        ...
        for epoch in range(model.params['training_step']):
            ...
            #if epoch % 10 == 0:
            #    model.critic.save_weights('./model/PPOcritic-epo'+str(epoch)+'.h5')
            #    model.actor.save_weights('./model/PPOactor-epo'+str(epoch)+'.h5')
            model.critic.save_weights('./model/PPOcritic.h5')
            model.actor.save_weights('./model/PPOactor.h5')
            ...
            np.save('./Results/'+str(trainID)+'_Train.npy',history)
            if epoch % 10 == 0:
                t2.append(time.perf_counter())

        timeR=[ti-t1 for ti in t2]
        print('Running time: ',timeR)
        np.save('./'+str(trainID)+'_time.npy',timeR)
        
        # plot
        plt.figure()
        plt.plot(history['Episode_reward'])
        plt.savefig('./Results/'+str(trainID)+'_Train.tif')
