...
if Train:
    ...
    buffer = Buffer.Buffer(model.params['state_dim'], 3*int(len(raindata[0])*model.params['num_rain']))
    ...
    # fusion different types of data
    for fusion_i in range(3):
        res = []
        ...
        for rainid in range(int(model.params['num_rain'])):
            ...
            if fusion_i == 0:
                optdata = np.load('./DN_results/dn_results_rain'+str(rainid)+'.npy',allow_pickle=True).tolist()
            elif fusion_i == 1:
                optdata = np.load('./DNv2_results/dnv2_results_rain'+str(rainid)+'.npy',allow_pickle=True).tolist()
            else:
                optdata = np.load('./BC2_results/bc2_results_rain'+str(rainid)+'.npy',allow_pickle=True).tolist()
            os.chdir(os.path.pardir)
            #print(os.getcwd())
            os.chdir('./step1-pretrainK123/Results_PPO_reward3')
            #print(os.getcwd())
            ...
            for ot,at,rt,vt,lo in zip(res[-1][0],res[-1][1],res[-1][2],res[-1][3],res[-1][4]):
                #buffer.store(o,a,r,vt,lo) # For OPT data
                atem = 0
                for it in range(agent_params['action_table'].shape[0]):
                    if (agent_params['action_table'][it] == np.array(at)).all():
                        atem = it
                buffer.store(ot,atem,rt,vt,lo)
            buffer.finish_trajectory(res[-1][5])
            sum_return += res[-1][6]
            sum_length += res[-1][7]
            num_episodes += 1
